...
Gateway 在读取前拒绝空或未知 scope，在计数前完成 scope 与字段过滤，最多返回十个候选。PON 台账地址搜索同样先按 Authorized OLT Scope 过滤，只投影 OLT identity、台账地址和 PON 坐标。用户实时状态组合接口只有在范围内恰好命中一个用户时才读取设备；PON 状态接口要求完整 PON 坐标，只投影最多 128 个 ONU 的坐标、快照姓名、在线状态和光功率，不返回电话、地址、LOID、MAC 或 SN。状态合同返回持久化的 opaque `datasetRevision`；完整用户快照替换、备份导入或影响用户资料的本机清洗会轮换版本。版本本身不由用户内容派生，不暴露用户资料或数据库实现。它不返回数据库、主机地址、凭据、NMSE 会话、项目、配置方案、审计或全量用户数据，也不提供同步和写操作。
