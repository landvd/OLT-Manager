...
- 从未注册 ONU/ONT 生成可复制的配置方案预览，包括 ZTE 自营上网、内部网络、自定义 VLAN、MDU+OTT，以及 Huawei 自营上网、内部网络模板。
...
- 配置方案依赖现场 VLAN 规划；ZTE 自定义 VLAN 需要人工输入业务 VLAN，MDU+OTT 需要从同 PON 已配置样板 ONU 或台账读取动态 VLAN。
