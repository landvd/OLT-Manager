...
- 配置方案弹窗支持复制命令和打开本机终端自动 Telnet 登录，方便人工粘贴确认。
- 首页作为运维概览，展示当前 OLT、SNMP 状态、未注册 ONU、异常 ONU、台账健康和快捷入口。
- 常用命令独立成命令手册页面，展示中兴 C300、华为 MA5800 常用配置命令，并支持按中文用途或命令片段模糊搜索。
- 管理本地 OLT 和 PON 台账，PON 台账支持页面编辑、Excel 导入导出和外层 VLAN 刷新。
- 记录 SNMP 测试历史和管理操作日志。
...
- 不做公网多用户管理平台。
- 不承诺所有厂商 OID 都已验证。
- Windows v1 不支持打开本机终端登录，也不支持 ZTE Expect Telnet 只读查询。
...
- 复制命令在内嵌浏览器中可用；打开终端可登录当前 OLT 并按厂商进入配置模式，但不自动粘贴或执行生成命令。
- PON 台账可导出为 Excel；Excel 导入只更新本地台账，不触发设备操作。
- 首页待处理事项只做只读统计和页面跳转，不自动处理 ONU 或写设备。
- 常用命令搜索只展示命令说明，不登录 OLT、不自动执行。
