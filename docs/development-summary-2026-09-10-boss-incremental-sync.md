# 2026-09-10：NMSE-PON BOSS 历史姓名初始化、只读增量与合并数据交接
...
6. 新增一次性历史姓名初始化：从 2019-08-23 至本次启动时间按月读取，按 LOID 建立最新姓名目录补全网管二期，完成后自动转入增量。
...
- 第 9 版 SQLite 迁移为旧库增加历史姓名状态和 `nmse_boss_name_snapshots`。初始化完成时间为空时，`nmse` 或 `full` 下一次运行自动进入历史模式。
- 历史模式冻结运行启动时间，从 `2019-08-23 00:00:00` 起按自然月分段，每段完整读取列表、全部分页和逐条详情。
- 历史阶段只保留 LOID、姓名、工单号和接收时间，不把过去的报装/移机/销户重放为当前坐标。同 LOID 取最新可归属姓名；不完整记录只计数跳过。
- 所有分段都成功后才用一个 `BEGIN IMMEDIATE` 事务替换姓名目录、更新一期源 revision/manifest 并推进水位。任一分段失败时不会留下半套初始化。
- 合并源以 LOID 使用该目录中的姓名，因此可覆盖网管二期的单字姓；姓名目录不覆盖网管二期坐标和设备字段。
- 2026-09-11 现场首轮历史读取暴露出外层 30 分钟租约只在阶段切换时续期的问题：17,818 条 NMSE 用户已读完并原子提交，但任务完成登记被过期租约拒绝。修复后任务开始或恢复即启动专用心跳，按不超过租约三分之一的间隔续租；姓名目录、网管二期源和统一快照提交前再次确认租约，成功、失败均清理定时器。
...
4. **自动化测试**：截至 2026-09-11，全量 558 项单元与集成测试 100% 通过（`pnpm test`），覆盖历史月份分段、LOID 姓名投影、空历史失败关闭、任一分段失败不提交、SQLite 迁移/姓名覆盖、租约心跳、跨进程单活跃运行和同运行提交重放。
5. **本轮验收边界**：上述 267 条和 13,977 条为改造前已完成的真实增量/合并基线；新增的 2019-08-23 历史姓名初始化本轮没有对真实 BOSS 执行，不得将合成测试当作现场全量验收。
6. **Win7 产物**：`release/OLT Manager-1.1.7-win7-x64.zip`，113,643,034 字节，SHA-256 `000b99668aa9291ad01d350cfe3db6d24eb3444b0adb9eb8255f99f5917b0b10`；ZIP 完整性、包内租约/飞书修复代码和 legacy SQLite 静态结构通过，未在真实 Win7 主机启动。
7. **macOS 产物**：`release/OLT Manager-1.1.7-arm64.dmg`，104,781,046 字节，SHA-256 `425b2cca7939090278f87bfa4fddd3c95bf8fff08af5c9180ed57a383bd5db91`；`hdiutil verify` 通过，当前仍是未签名、未公证的 Apple Silicon 内部分发包。
...
3. 旧库升级后，按钮首次显示 **一期 BOSS 历史全量初始化**：
   - 系统按月从 2019-08-23 读取到本次启动时间，可通过页面查看历史批次和当前分页。
   - 运行中不要重复点击或退出应用；失败后可安全重试，不会保留半套姓名。
   - 成功后显示收录的 LOID 姓名数，按钮自动改为 **一期 BOSS 增量同步**。
   - 后续增量基于上次成功 Watermark 向前重叠 1 天，读取至本次启动时间。
4. 点击 **手动合并**（或执行全量同步）：
   - 合并引擎将本地 BOSS 快照与网管二期快照做最终关联合并，更新 `merged_onu_snapshots`。

### 4.2 异常排查
- **历史读取结束后提示租约失效**：升级到 2026-09-11 续租修复包。若页面已显示历史姓名数，说明一期原子源可能已经提交；再次发起一期同步会根据本地完成标记和 watermark 走短增量，不重跑 2019 年以来的历史。该任务成功后再执行手动合并。
...
- 单元测试集：[`tests/nmse-boss-sync.test.mjs`](file:///Users/mac/Documents/OLT%20Manager/tests/nmse-boss-sync.test.mjs)、[`tests/nmse-boss-db.test.mjs`](file:///Users/mac/Documents/OLT%20Manager/tests/nmse-boss-db.test.mjs)、[`tests/nmse-boss-name-history-db.test.mjs`](file:///Users/mac/Documents/OLT%20Manager/tests/nmse-boss-name-history-db.test.mjs)
