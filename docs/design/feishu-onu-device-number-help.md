...
默认查询顺序为：姓名 → 手机 → LOID → 设备号 → 装机地址。前一字段没有匹配时继续查询下一字段；全部字段没有匹配，返回帮助菜单而不是“没有找到匹配项”。唯一匹配返回 ONU 详情，多条匹配返回可分页候选；输入“帮助”或“help”可重新显示说明。ONU 序列号/SN、MAC、ONU 坐标和 PON 地址等专用格式仍由已配置的语言解析器识别。
