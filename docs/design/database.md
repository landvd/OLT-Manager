...
用户与 VLAN 快照均是本地运行数据，不得提交。用户同步先读取第 1 页确定总量，剩余页最多 8 路独立会话并发读取；只有完整远端分页全部成功后才以事务替换同 OLT 旧快照。正式快照和调试检查点写入前都调用 `normalizeResourceInstallationAddress()` 清洗装机地址：去除末尾 `#`，压缩已知重复的片区/镇村前缀，并保留有效的“厚街镇”等行政区名称。检查点仅替换同 OLT 旧检查点。VLAN 同步只更新本地已存在且板卡/PON 匹配的台账行。
