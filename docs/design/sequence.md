...
sequenceDiagram
  ...
  API-->>Browser: 展示历史表格
  Note over DB,OSS: 有迁移主密码时 DB 保存跨平台登录密文；免主密码模式可保存本机密码，普通备份因此属于敏感文件；桌面自动登录凭据可由系统加密存储在 SQLite 外；会话/CUID仍只在内存，不刷新光功率，不访问 OLT 写接口
```

历史查询在没有现存 OSS/NGB 会话时，依次使用操作系统加密凭据、本机 SQLite 免主密码登录材料或用户提供的迁移主密码按需建立只读会话；会话自成功登录起最多保留 10 分钟，随后自动丢弃本地会话和 Cookie 引用。历史查询只能在同一已授权会话中使用固定 OLT CUID/ONU CUID 和固定模板读取，并先投影再返回。应用关闭、配置变更、显式退出或 401 会话失效都会清理会话。验证码和多登录部门选择仍按失败关闭处理。
