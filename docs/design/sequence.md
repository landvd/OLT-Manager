...
一期 NMSE-PON 的“同步”按钮改为在现有 `/api/admin/merged-onu/sync/nmse` 受控运行中调用 BOSS 只读增量入口：先以 `/BOSS/BOSSInstruction` 页面建立只读会话，再查询固定使用“处理状态=成功、操作状态=全部、查询内容=厚街镇”（成功字典值 `2`、全部字典值 `0`），BOSS `ConversionDate` 使用上海墙钟的非补零格式；D 日 eligibleEnd 为 D-1 00:00，起点按上次成功 watermark 向前重叠一天。结果按工单号/LOID/接收时间幂等；报装、移机、更换 ONU 更新本地一期快照，只有成功销户才安全删除。远端查询、逐条详情、备份、字段投影、事件应用和 watermark 更新必须在成功事务完成后推进，失败保留旧快照和旧水位。
...
一期事务是独立的 staged source commit：事件、快照、`coverage_through`、来源状态和 manifest 一起原子提交并推进 exclusive watermark；随后网管二期源同步或统一合并失败，不回滚已经成功的一期源，而是保留可重试、可单独展示的已确认一期源状态。旧 v1 manifest 不按 v2 语义解释；两套源均重新成功生成 v2 后才能合并。
...
桌面版勾选本机自动登录后，历史查询在没有现存 OSS/NGB 会话时使用操作系统加密凭据按需建立只读会话；会话自成功登录起最多保留 10 分钟，随后自动丢弃本地会话和 Cookie 引用。纯 Web/Node 环境或跨设备迁移仍由用户输入迁移主密码解密 SQLite 密文；未提供可解锁材料时历史查询失败关闭。历史查询只能在同一已授权会话中使用固定 OLT CUID/ONU CUID 和固定模板读取，并先投影再返回。应用关闭、配置变更、显式退出或 401 会话失效都会清理会话。当前切片不提供 OLT 列表管理界面、不自动创建 IP 映射、不保存历史明细，也不接入定时任务；验证码和多登录部门选择仍按失败关闭处理。
