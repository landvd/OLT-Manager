...
4. GitHub Actions 会运行 `.github/workflows/release.yml`：
   ...
   - `windows-2022` 准备 `bin/win32/sqlite3.exe` 后构建 Windows x64 NSIS 安装包和 ZIP 包。
...
- 当前已发布 `1.0.0`；后续修复补丁升级 `1.0.1`，小功能升级 `1.1.0`，重大不兼容变化升级下一个主版本。
...
- Windows 7 包内的 SQLite CLI 优先位于 `resources/app/bin/win32/sqlite3.exe`，NSIS 安装版另通过 `extraResources` 保留 `resources/bin/win32/sqlite3.exe`；Electron 启动本地服务前会自动把存在的绝对路径设置为 `OLT_MANAGER_SQLITE_BIN`。
...
- SQLite：macOS 优先使用系统 `/usr/bin/sqlite3`；Windows 7 x64 发行包内置 `bin/win32/sqlite3.exe`。该文件应通过 `pnpm run prepare:win-sqlite` 准备固定的 SQLite 3.41.0 Windows x86 CLI，避免较新的 x64 CLI 在 Win7 上触发 `0xC0000139` entry-point 错误。安装版会自动绑定包内路径，不需要加入 PATH；也可通过 `OLT_MANAGER_SQLITE_BIN` 覆盖。
...
- 设备相关：Win7 安装版诊断日志中 `sqliteBin` 应指向 `resources/app/bin/win32/sqlite3.exe` 或 `resources/bin/win32/sqlite3.exe`；缺少 SNMP/SQLite 工具时页面返回清楚错误。
