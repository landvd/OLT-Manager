const { createApp, computed, nextTick, onMounted, reactive } = Vue;
...
function parsePonImport(text) {
  ...
  return trimmed.split(/\r?\n/)
    ...
    .map(([oltIp, ponPort, address]) => ({ oltIp, ponPort, address: address || "" }));
}
...
const App = {
      ...
      <el-container>
        ...
        <el-main class="app-main">
          ...
          <section v-else-if="state.activeView === 'adminPonPorts'">
            ...
            <el-card shadow="never" class="content-card">
              <div class="pon-tools">
                <el-input v-model="state.ponAdminSearch" clearable placeholder="搜索 OLT/IP/PON/地址" />
  ...
  setup() {
    const state = reactive({
      ...
      ponAdminSearch: "",
      ponImportText: "",
      loading: { status: false, install: false, onus: false, admin: false }
    });
    ...
    const filteredPonPorts = computed(() => {
      ...
      return state.ponPorts
        .map((port, index) => ({
          port,
          __index: index,
          searchText: `${port.oltIp || ""} ${port.ponPort || ""} ${port.address || ""}`.toLowerCase()
        }))
    ...
    function applyAddressSearchToPon() {
      ...
      const match = currentPonPorts.value
    ...
    async function loadAdminData() {
      ...
      try {
        const [olts, history, events] = await Promise.all([
    ...
    function addPonPort() {
      state.ponPorts.unshift({ oltIp: selectedOlt.value.host || "", ponPort: "", address: "" });
    ...
    async function savePonPorts() {
      ...
      try {
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "保存失败");
        state.ponPorts = await fetch("/api/admin/pon-ports").then((item) => item.json());
