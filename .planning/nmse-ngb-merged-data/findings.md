...
- 2026-09-02 最终由现场页面确认：NMSE-PON 上游资源信息已经消失。登录与 OLT 发现仍可成功，但 ONU 列表接口因缺少上游资源返回空 `HTTP 500`；这不是 OLT Manager 分页、Cookie 或 Win7/Mac 传输回归。针对该现象制作的请求头、页码回退、多 Cookie 与 keep-alive 试探性补丁均不保留，以 `v1.1.4` 至原始 `v1.1.6` 相同的客户端实现为基线。
