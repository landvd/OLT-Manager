...
`src/olt-data-gateway.mjs` 是面向外部只读查询应用的深模块。它把 SQLite 用户快照、OLT inventory 与现有厂商只读采集隐藏在四个稳定 Interface 后：`status`、`listOlts`、`queryUsers`、`readOnuStatus`。`status` 返回持久化、opaque、非敏感的 `datasetRevision`；完整用户快照替换、导入、清空或会改变用户资料的本机清洗会轮换该版本，使消费者能够使旧的数据集确认失效，但不能从版本反推出任何用户字段。HTTP Adapter 仅暴露 `/api/gateway/v1/*`，默认随主服务绑定 `127.0.0.1`，并要求独立 `OLT_MANAGER_GATEWAY_TOKEN` bearer；未配置即禁用。
