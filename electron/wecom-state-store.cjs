...
const STATE_FORMAT = "olt-manager/wecom-state-store/v1";
const KEY_FORMAT = "olt-manager/wecom-state-key/v1";

function createWecomStateStore({ dataDirectory, safeStorage }) {
  const statePath = path.join(dataDirectory, "wecom-state.enc");
  const keyPath = path.join(dataDirectory, "wecom-state-key.json");
  ...
  async function readKey() {
    if (!safeStorage.isEncryptionAvailable()) {
      throw new Error("WeCom state OS encryption unavailable");
    }
    try {
      ...
      if (error.code !== "ENOENT") throw new Error("WeCom state key unavailable", { cause: error });
  ...
  return Object.freeze({
    ...
    async read() {
      ...
      try {
        ...
        throw new Error("WeCom state authentication failed", { cause: error });
      }
    }
  });
}

module.exports = { createWecomStateStore, KEY_FORMAT, STATE_FORMAT };
