...
## 2026-09-12 中兴 C600 (ZXA10-TITAN) SNMP OID 与 MIB 兼容性只读对比实验

- 设备/系统别名：`zte-c600-site-a` (实测设备：C600 V2.0.10)
- 目标：验证中兴 C600 的 SNMP OID 与 MIB 结构是否与既有中兴 C300 相同；若不相同，定位 C600 实现同等只读管理功能（状态探活、PON 端口识别、ONU 列表、SN、运行状态等）所需的专用 OID 与表结构。
- 操作类型：SNMP v2c `get/walk` 只读查询；未执行 `snmpset`、未发送配置修改或写命令。
- 是否只读：是。

### 观察与对比结论

1. **基础管理 MIB (RFC 1213 MIB-2)：完全相同且兼容**
   - `sysDescr` (`1.3.6.1.2.1.1.1.0`)：正常响应（`ZXA10 C600, ZTE ZXA10 Software Version: V2.0.10`）。
   - `sysObjectID` (`1.3.6.1.2.1.1.2.0`)：正常响应（`1.3.6.1.4.1.3902.1082.1001.600.1.1`，标识 C600 企业节点）。
   - `sysUpTime` (`1.3.6.1.2.1.1.3.0`)：正常响应（以百毫秒为单位正常自增）。
   - `sysName` (`1.3.6.1.2.1.1.5.0`)：正常响应（返回设备规划名）。
   - `ifName` (`1.3.6.1.2.1.31.1.1.1.1`)：正常响应（标准端口名称，如 `gpon_olt-1/1/1` ~ `15`）。

2. **企业私有 XPON 管理 MIB (`1.3.6.1.4.1.3902.*`)：架构不相同（平台换代）**
   - **C300 早期架构**：使用 `1.3.6.1.4.1.3902.1012` 分支（`ZXA10-EPON/GPON-MIB`），包括 `1012.3.28.1.1.3` (onuName)、`1012.3.28.1.1.5` (SN)、`1012.3.28.2.1.4` (phaseState) 等；这些 OID 在 C600 上全量返回空（已废弃）。
   - **C600 TITAN 架构**：全面迁移至 `1.3.6.1.4.1.3902.1082` 分支（`ZX-XPON-MIB` / `ZX-PON-MIB`）。
   - 实测定位的 C600 核心映射关系如下：
...
- 设备/系统别名：`zte-c600-site-a` (实测设备：C600 V2.0.10)
- 目标：通过只读 Telnet 读取现网运行中 ONU 的配置语法，与既有 C300 模板对比，提炼 C600 专属配置方案模板并进行安全隔离。
- 操作类型：只读 Telnet 连接、进入用户/特权视图后只读执行 `show running-config` / `show this` 命令。
- 是否只读：是。未向设备写入、修改、下发任何配置，未保存配置。

### 现场命令实测与观察

1. **进入配置模式**：
   - 尝试 `con t`：系统返回 `%Error 140301: Ambiguous command:"con t"`，提示命令有歧义。
   - 验证：必须完整输入 `configure terminal`。
2. **物理/逻辑接口命名**：
   - C300 命名规范为：`interface gpon-olt_1/1/1` 与 `interface gpon-onu_1/1/1:1`。
...
4. **智能网关透传接口**：
   ...
   - C600 现代智能家庭网关/一体机（如测试中的中兴/天邑智能网关）默认使用 VEIP 虚拟以太口：
     ```text
     vlan port veip_1 mode trunk
     vlan port veip_1 vlan 3301
     ```
5. **配置查看命令**：
   - C300 可以在任意视图执行 `show running-config interface ...` 查看。
   - C600 跨视图直接查看报错，现场必须进入具体接口视图下执行 `show this` 查看当前接口配置。
...
- 设备/系统别名：`wecom-robot-gateway` (企业微信官方 OpenAPI WebSocket 服务：`wss://openws.work.weixin.qq.com`)
- 目标：验证企业微信 API 模式 WebSocket 长连接在无公网 IP、无公网端口映射环境下的稳定性，验证与核心只读数据网关 `OltDataGateway` 的接驳效果，并解决一级地址地名（如“坑口”）与用户装机门牌地址的优先级冲突。
- 操作类型：WebSocket 全双工长连接订阅、消息接收与分发、进程内 `OltDataGateway` 只读查询、本地 SQLite 台账与 SNMP 只读读取；未向 OLT 或企业微信执行任何写操作。
- 是否只读：是。未修改 OLT 配置，未调用写接口。

### 现场流程与观察

1. **安全配置与连接握手验证**：
   - 现场配置企业微信自建应用智能机器人的 Bot ID 与 Secret；
   - 凭据经由 Electron `safeStorage` 或 `AES-256-GCM` 密文持久化至状态文件，日志与网络包均脱敏；
   - WebSocket 长连接建链成功后收到系统 `ready` 事件，测试发送 `help`，收到四维运维排障完整欢迎指南。
2. **姓名查单与装维模糊消歧验证**：
   - 输入“田金水”：命中唯一 ONU 档案，秒级返回光功率红绿灯仪表盘（🟢 良好）、在线状态、所属 OLT、板卡/PON 坐标、LOID、MAC、电话直拨及装机地址；
   - 底部自动附加移动端会话快捷指令引导（回复 `历史`、`整口`、`LOID`）。
3. **一级地址反查 PON 口与装机门牌地名冲突实测**：
   - 现象：输入“坑口”时，旧逻辑直接进入模糊用户查单，由于大量用户装机地址包含“坑口村X巷X号”，导致直接匹配出了住户列表，而装维人员的真实意图是查看覆盖该片区的一级地址 PON 口整口态势；
   - 验证优化：调整查询优先级，凡地名/地址特征输入，优先调用 `gateway.queryPons` 检索 PON 台账一级地址。
   - 结果：
     - 若该地名对应唯一 PON 口：直出该 PON 口全量 ONU 态势卡片（在线率、离线分布、弱光排查）；
     - 若该地名对应多个 PON 口（如厚街坑口有 3 个 PON 口覆盖）：直出序号选单列表（1/2/3 对应各端口及光衰统计）；回复数字序号 `1` 即可秒开该端口态势；
     - 若装维确实要查“坑口”的用户：支持显式输入 `用户 坑口` 强制进入用户列表检索。
4. **7 天历史光衰时序与连续会话快捷指令实测**：
   - 用户在查看 ONU 详情后，直接在手机键盘回复 `历史`：系统从 15 分钟会话上下文提取 `lastUser`，读取最近 7 天最多 48 条光衰历史，生成波动极差、日均值、最高/最低光衰及稳定性定界结论（主干正常/突变劣化/皮线老弱光）；
   - 回复 `整口`：自动提取该用户所属 OLT 与 PON 坐标，直出整口态势卡片；
   - 回复 `LOID`：单独回复易于长按复制的纯文本 LOID。

### 结论与交付规则

1. **网络与安全边界**：企业微信 WebSocket 全双工长连接极速稳定，纯单向出站连接完全避免了公网暴露风险。
2. **消歧规则固化**：地名/地址输入优先作为一级地址反查 PON 口；用户查询需通过人名、LOID、设备号或显式 `用户 <地名>` 触发。
3. **移动端键盘交互优于卡片回调**：在无按钮事件回调的企业微信 API 模式下，基于 15 分钟 TTL 内存会话状态机实现的“数字序号秒开（1-10）”与“快捷短语（历史/整口/LOID）”盲打交互，大幅提升了装维现场的排障速度。

## 2026-09-14 华为 MA5800 机框 0 坐标纠偏与多村穿透借测只读验证

- 设备/系统别名：`huawei-ma5800-site-dajing` (厚街机房华为 MA5800-X15，IP `172.19.106.51`)
- 目标：验证大迳村巡检报 `172.19.106.51 1/1/1未完成` 的根因，验证机框号纠偏机制以及目标村用户全离线场景下的同口在线借测机制。
- 操作类型：SNMP v2c GET/WALK 读取（`ifName`、`ontDescription`、`ontSerialNumber`、`rxPower`）；未执行任何配置命令。
- 是否只读：是。

### 现场观察与根因分析

1. **机框命名差异（中兴 vs 华为）**：
   - 中兴 C300 ROS 规范机框号为 `1`（`gpon-olt_1/x/y`）；
   - 华为 MA5800-X15 单机框部署时主框固定为 `0`（`GPON 0/1/1`）。
   - 现场历史台账将大迳村该华为端口录入了机框 `1`（`1/1/1`）。网关调用 SNMP Walk 时使用 `1/1/1` 匹配华为 MIB `ifName` 时查空，导致无任何 ONT 返回，最终被扣上“未完成”状态。
2. **解决与验证**：
   - 在网关 `queryVillagePonsImpl` 与 `sampleVillagePonOnlineUser` 中加入厂商自适应检测：当目标 OLT 为华为（`vendor === "huawei"`），台账记录机框为 `1` 时自动自适应纠偏为 `0`（`0/1/1`）。
   - 实机验证：纠偏后成功匹配 `GPON 0/1/1`，顺利读取在线 ONT 实时光衰（`-21.4 dBm`），大迳村该端口由“未完成”恢复为“正常完成”。
3. **同口其他村在线用户穿透借测**：
   - 双岗村等边缘端口在白天出现目标村台账用户（1~2 户）离线或搬迁；
   - 移除了 `!matchedUsers.length` 的阻断，借测同口其他在线用户（`firstOnline`）测得光衰，证明主干与分光器通光正常，消除伪未完成。

## 2026-09-14 Pi Agent 弱光聚类定界诊断与离线原因研判决策树只读实测

- 模块别名：`pi-agent-diagnostic-engine`
- 目标：验证整口光功率聚合分析时的聚类故障定界准确性，以及单户掉线时通过下线原因代码（`Offline Reason`）快速定界的装维决策支持。
- 操作类型：进程内数据网关只读聚合、SNMP 只读历史、确定性推理；无设备写操作。
- 是否只读：是。

### 1. 整口弱光聚类三级定界法则实测

- **法则设计**：
  - 🔴 **整口主干级故障**（弱光率 $\ge 50\%$ 或均值 $< -26.5\text{ dBm}$）：判定为机房至一级光交箱主干大衰耗或一级分光器损坏，**【严禁盲目入户修线】**，指引测量 OLT 发光功率并排查一级主干法兰；
  - 🟡 **分支二级箱级故障**（弱光率 $20\% \sim 50\%$）：判定为特定楼栋或二级分光器法兰脏污，指引排查二级分光箱；
  - 🟢 **散发性单户故障**（弱光率 $< 20\%$，仅 1~2 户）：判定为主干与分光优良，仅需入户重做冷接子或更换尾纤；
  - 🟢 **全口优良**（0 户弱光）：全口余量充足，无需物理维护。
- **实测结果**：在测试用例中注入 4 类典型端口样本，均 100% 精准判定为对应级别并给出针对性装维指引。

### 2. 离线原因代码与现场决策树实测

- **决策树验证**：
  - ⚡ **用户侧掉电关机（DyingGasp / code=2）**：100% 证明物理光纤畅通，切勿盲目上门翻动光纤；优先核实用户拔电或跳闸，排查 12V 适配器电容鼓包；
  - 🚨 **光路物理中断（LOS / code=3）**：通电突然失光，需携带红光笔上门排查皮线与法兰；
  - ⚠️ **帧失步严重劣化（LOF / code=4）**：光衰临界失步；
  - 🔄 **频繁闪断震荡（Flapping）**：短期多次下线，排查接触不良或风吹晃动。
- **实测结果**：通过 `tests/pi-agent.test.mjs` 验证，输入 DyingGasp 与 LOS 样本均正确输出对应的行动指引，杜绝了无序上门与误动光缆。

## 2026-09-15 华为 OLT 网管二期机框 0 中文格式解析缺陷与存量快照对齐验证

- 设备/系统别名：`huawei-ma5800-site-102` (`172.19.104.102`)、网管二期（OSS/NGB OnuGridBO）与一期 BOSS（NMSE-PON）
- 目标：排查华为 OLT 在二期网管同步后以 `1/1/0` 开头而非真实物理坐标 `0/1/0` 的 BUG，修复解析与归一化逻辑，纠偏已有数据库快照并完成全量重新对齐合并。
- 操作类型：代码修复、进程内数据归一化、SQLite 安全事务更新与全量 Manual Merge；未向设备下发写配置。
- 是否只读：设备侧只读，本地数据库完成受备份保护的纠偏与全量合并。

### 现场观察与根因分析

1. **错位现象**：
   - 华为 MA5800 设备实际物理坐标为机框 0（Frame 0）、槽位 1（Slot 1）、PON 口 0（Port 0），标准坐标为 `0/1/0:x`；
   - 检查本地数据库 `merged_onu_network_snapshots` 中关于 `172.19.104.102` 的 1227 条记录，发现有 1225 条被错误记为 `1/1/0:x`（`chassis = 1`），仅 2 条为 `0`；
   - 导致严重后果：一期 BOSS 中华为用户资料以 `0/1/0:x` 存储，物理设备 SNMP 端口为 `GPON 0/1/0`；网管快照中的 `1/1/0:x` 导致三方物理坐标完全错位，全部华为用户姓名与资料匹配失败。

2. **根因定位**：
   - **未适配中文命名**：网管二期在拉取华为 OLT 时，`DEVNAME` 格式为 `172.19.104.102/框0/槽1/端口0/OnuID0`。现存正则 `/(?:^|\s)(\d+)\/(\d+)\/(\d+):(\d+)(?:\s|$)/` 仅支持纯数字斜杠冒号格式（如中兴 `1/12/1:1`），遇中文直接匹配失败；
   - **回退逻辑硬编码**：正则失败后回退至 `OLTCARDIDX`、`OLTPORTIDX`、`ONUIDX` 时，代码中写死了 `return { chassis: "1", board, pon, onuId };`，将华为机框强行覆盖为 `"1"`。

### 解决与实机验证

1. **代码修复**：
   - `src/oss-ngb-client.mjs` 中增加中文模式正则匹配：
     `/(?:^|[^\d])框\s*(\d+)\s*[/_\s-]*\s*槽(?:位)?\s*(\d+)\s*[/_\s-]*\s*(?:PON\s*口?|端口?|口)\s*(\d+)\s*[/_\s-:]*\s*(?:OnuID|ONU|ONT)?\s*(\d+)(?:\D|$)/i`；
   - 回退逻辑支持识别机框字段，未识别时根据 `vendor` 动态推导（华为 `0`，中兴 `1`）；
   - `src/merged-onu-sync.mjs` 的 `normalizeMergedCoordinate` 同步增强中文格式匹配。
2. **存量数据库原地纠偏与合并实测**：
   - 执行纠偏脚本 `scripts/repair-huawei-network-coordinates.mjs`：
     - 生成备份：`olt-manager-before-huawei-chassis-repair-*.sqlite`；
