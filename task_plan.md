...
新增“备份还原”菜单，一键导出和导入完整本机项目 SQLite 数据。导入仅替换本机数据，不触发任何 OLT 命令。

### 阶段

22. [complete] 梳理数据库路径、桌面运行时路径与现有 SQLite 访问串行化方式。
23. [complete] 设计备份包格式、导出下载、导入校验和原子替换。
24. [complete] 实现后端 API、前端菜单与确认提示。
25. [complete] 运行现有完整回归测试；备份 API 专项恢复样例待后续补充。
26. [complete] 更新文档、验证构建并重启本地服务。

## 新增任务：用户装机地址清洗

27. [complete] 抽样检查本机用户快照地址并定义重复地址清洗规则。
28. [complete] 实现可重复执行的本地地址清洗操作与预览结果。
29. [complete] 运行清洗、验证更新行数并记录用户可见变化。

### 安全边界

- 备份包含完整本机项目数据，可能含 OLT 和资源系统凭据；只允许用户手工保存到可信位置。
- 导入必须显式确认，先校验 SQLite 完整性，再原子替换本机数据库。
- 导入导出绝不触发 SNMP、Telnet 或任何 OLT 写操作。
