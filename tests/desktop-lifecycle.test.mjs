...
const rendererResourceSyncApi = await readFile(new URL("../src/resource-sync-api.mjs", import.meta.url), "utf8");
...
test("desktop lifecycle keeps platform targets, user-data paths, and no-publish release boundaries", () => {
  ...
  assert.match(electronMain, /assets", "generated", "olt-manager-16\.png/);
  assert.match(electronMain, /assets", "generated", "olt-manager\.ico/);
  ...
  assert.match(rendererMergedOnuState, /尚未同步/);
  assert.match(rendererResourceSyncApi, /api\/admin\/merged-onu\/sync/);
  assert.match(rendererResourceSyncApi, /api\/admin\/merged-onu\/sync\/network/);
  assert.match(rendererResourceSyncApi, /api\/admin\/merged-onu\/sync\/nmse/);
  assert.match(rendererResourceSyncApi, /api\/admin\/merged-onu\/merge/);
  ...
  assert.match(rendererDashboardState, /formatUptime\(status\.uptime\)/);
  assert.match(rendererResourceSyncApi, /api\/admin\/resource-sync-tasks/);
  assert.match(rendererMain, /repeatEnabled/);
  assert.match(rendererMain, /repeatDays/);
  assert.match(rendererScheduleState, /每 \$\{task\.repeatDays\} 天/);
  assert.match(rendererMain, /deleteResourceSchedule/);
  assert.match(rendererResourceSyncApi, /resource-sync-tasks\/.*\/delete/);
