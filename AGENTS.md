...
- **语言要求**：所有 AI 智能体（Antigravity cli）在与用户对话、输出技术日志、提示错误或编写代码注释时，**必须全程、无条件使用中文交流**。
- **术语例外**：对于编程语言、框架、库、API、协议、设计模式等专用术语（如 `React`、`RESTful`、`Docker`、`GraphQL` 等），允许保留英文原文，无需强行翻译，以保证技术表达的准确性和可读性。
